...
func WrapHandler(handler *webdav.Handler, options ...func(*Config)) http.HandlerFunc {
	var config = Config{
		URL:                      "doc.json",
		DocExpansion:             "list",
...
func CustomWrapHandler(config *Config, handler *webdav.Handler) http.HandlerFunc {
	var once sync.Once
	...
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
			httpx.ErrorCtx(r.Context(), w, errors.New(http.StatusMethodNotAllowed, "Method not allowed"))
			return
		}

		matches := matcher.FindStringSubmatch(r.RequestURI)

		if len(matches) != 3 {
			httpx.ErrorCtx(r.Context(), w, errors.New(http.StatusNotFound, "404 page not found"))
			return
		}
		...
		switch path {
		case "index.html":
			_ = index.Execute(w, config.toSwaggerConfig())
		case "index.css":
			_ = css.Execute(w, config.toSwaggerConfig())
		case "swagger-initializer.js":
			_ = js.Execute(w, config.toSwaggerConfig())
		case "doc.json":
			doc, err := swag.ReadDoc(config.InstanceName)

			if err != nil {
				httpx.ErrorCtx(r.Context(), w, errors.New(http.StatusInternalServerError, "Server Internal Error"))
				return
			}

			var obj map[string]interface{}
			_ = json.Unmarshal([]byte(doc), &obj)
			httpx.OkJson(w, obj)
		default:
...
func DisablingWrapHandler(handler *webdav.Handler, envName string) http.HandlerFunc {
	if os.Getenv(envName) != "" {
		return func(w http.ResponseWriter, r *http.Request) {
			// Simulate behavior when route unspecified and
			// return 404 HTTP code
			httpx.ErrorCtx(r.Context(), w, errors.New(http.StatusNotFound, ""))
		}
	}
...
// if specified environment variable passed.
func DisablingCustomWrapHandler(config *Config, handler *webdav.Handler, envName string) http.HandlerFunc {
	if os.Getenv(envName) != "" {
		return func(w http.ResponseWriter, r *http.Request) {
			// Simulate behavior when route unspecified and
			// return 404 HTTP code
